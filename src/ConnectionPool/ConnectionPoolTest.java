...
public class ConnectionPoolTest {
	...
	void addConnection() {
		...
		assertEquals(pool.listNodes.head.connection, pool.getConnection(55));
		...
		// checked the first element
		assertEquals(pool.listNodes.head.connection, pool.getConnection(66));

		// checked the last element
		assertEquals(pool.listNodes.tail.connection, pool.getConnection(22));
	...
	void getConnection() {
		...
		assertEquals(pool.listNodes.head.connection, con55);

		// take the penultimate element and check that it became the last
		Connection con44 = pool.getConnection(44);
		assertEquals(pool.listNodes.head.connection, con44);

		assertEquals(33, pool.getConnection(33).id);
		assertEquals(pool.listNodes.head.connection, pool.getConnection(33));
